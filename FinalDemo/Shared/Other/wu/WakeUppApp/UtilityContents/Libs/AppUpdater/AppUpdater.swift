...
public class AppUpdater: NSObject {
    ...
    public class func showUpdateAlert(isForce:Bool = false) {
        ...
        if compare(appstoreVersion: data.version) {
            alert = UIAlertController(title: applicationName, message: "Version \(data.version) is available on the AppStore. Please update the version.", preferredStyle: UIAlertControllerStyle.alert)
