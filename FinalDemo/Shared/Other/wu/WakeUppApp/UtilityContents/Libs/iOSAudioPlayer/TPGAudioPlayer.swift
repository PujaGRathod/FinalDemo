    ...
    public func playPauseMediaFile(audioUrl: NSURL, springboardInfo: Dictionary <String, AnyObject>, startTime: Double, completion: @escaping (_ previousItem: String?, _ stopTime: Double) -> ()) {
        ...
        DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async {
            ...
            if url.contains("http"){
                playerAsset.resourceLoader.setDelegate(self, queue: DispatchQueue.main)
            }
    ...
    @objc func playerDidReachEnd(_ notification: NSNotification) {
        ...
        if currentItem?.asset is AVURLAsset{
            ...
            if "\(url)".contains("http"){
                ...
                filename = filename.replacingOccurrences(of: "mp3", with: "m4a")
                filename = filename.replacingOccurrences(of: "wav", with: "m4a")
                ...
                if isFileLocallySaved(fileUrl: localURL) == false{
                    //showHUD()
 ...
 */
extension TPGAudioPlayer : AVAssetResourceLoaderDelegate{
