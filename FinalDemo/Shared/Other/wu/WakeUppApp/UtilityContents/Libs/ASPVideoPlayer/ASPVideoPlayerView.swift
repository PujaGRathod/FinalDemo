...
@IBDesignable open class ASPVideoPlayerView: UIView {
    ...
    /**
     The URLAsset that should be loaded and cached.
     */
    open var urlVideoAsset: AVURLAsset? = nil {
        didSet{
            guard let asset = urlVideoAsset else {
                status = .error
    ...
    /**
     The video composition to be provided to AVPlayerItem
     */
    ...
    private func setVideoAsset(asset: AVAsset) {
        ...
        if asset is AVURLAsset{
            ...
            if url.contains("http"){
                (asset as! AVURLAsset).resourceLoader.setDelegate(self, queue: DispatchQueue.main)
            }
        }
    ...
    @objc internal func itemDidFinishPlaying(_ notification: Notification) {
        ...
        if currentItem?.asset is AVURLAsset{
...
/**
 For AVURLAsset Caching
 */
extension ASPVideoPlayerView : AVAssetResourceLoaderDelegate{
