   ...
   @objc func tapGesture(_ recognizer: UITapGestureRecognizer) {
    ...
    if let view = recognizer.view {
            if view is UIImageView {
                //Tap only on visible parts on the image
                //for imageView in subImageViews(view: canvasImageView) {
                for imageView in subImageViews(view: cell.canvasImageView!) { //PV
                    let location = recognizer.location(in: imageView)
                    let alpha = imageView.alphaAtPoint(location)
                    if alpha > 0 {
    ...
    func moveView(view: UIView, recognizer: UIPanGestureRecognizer)  {
        ...
        /*view.centeq r = CGPoint(x: view.center.x + recognizer.translation(in: canvasImageView).x, y: view.center.y + recognizer.translation(in: canvasImageView).y)
        recognizer.setTranslation(CGPoint.zero, in: canvasImageView)*/
        
        //PV
        view.center = CGPoint(x: view.center.x + recognizer.translation(in: cell.canvasImageView).x, y: view.center.y + recognizer.translation(in: cell.canvasImageView).y)
        ...
        if let previousPoint = lastPanPoint {
            ...
            if deleteView.frame.contains(pointToSuperView) && !deleteView.frame.contains(previousPoint) {
                ...
                UIView.animate(withDuration: 0.3, animations: {
                    ...
                    view.center = recognizer.location(in: cell.canvasImageView) //PV
                })
            }
        ...
        if recognizer.state == .ended {
            ...
            } else if !cell.canvasImageView.bounds.contains(view.center) { //PV
                UIView.animate(withDuration: 0.3, animations: {
                    //view.center = self.canvasImageView.center
                    view.center = cell.canvasImageView.center //PV
                })
