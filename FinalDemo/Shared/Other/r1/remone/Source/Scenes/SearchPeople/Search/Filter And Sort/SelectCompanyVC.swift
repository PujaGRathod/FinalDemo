...
class SelectCompanyVC: UITableViewController {

    var selectionMandatory: Bool = true

    var companies: [RMCompany] = []
    var selectedCompany: RMCompany?
    var backClosure: ((RMCompany?)-> Void)?
    var filteredCompanies = [RMCompany]()

    @IBOutlet weak var searchTextField: UITextField!

    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Company".localized
        Analytics.shared.trackScreen(name: "Select Company")
    ...
    func getAllCompanyList()  {
        self.showLoader()
        APIManager.shared.fetchActualCompanyList{ (result, error) in
            self.hideLoader()
            if let error = error {
